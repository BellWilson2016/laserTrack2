function gradientFreqSeriesY() 

	global allScheduledEvents;
	allScheduledEvents = [];	% Clear existing schedule

	% Setup generic experimental info
	exp.experimentName = [datestr(now,'YYmmDD-HHMMss-'),'gradientFreqSeriesY'];
	...
	exp.laserPowers    = [0,5,20,50]; % Work as frequencies in Hz
	%exp.laserPowers    = [108,162];
	exp.laserFilter    = 1;
	exp.nReps          = 16;
	exp.comment		   = '2D arena, Y gradient';	
	...
	for repN = 1:exp.nReps
		...
		for seqN = 1:nSeq
			...
			exp.protocolArgs = {@laserLinearFreqGradientY, [powerL, powerR]};
