function singleSideSeriesShortRBLP() 
	...
	exp.experimentName = [datestr(now,'YYmmDD-HHMMss-'),'singleSideSeriesShortRBLP'];
    exp.genotype       = 'NorpA[7]/y ; H134R / + ; Or42a-Gal4 / +';
    exp.flyAge         = 8;    % Days
    exp.sex            = 'M';
    exp.odor           = 'none';
	...
	exp.laserPowers    =  [0,2,3,4,5,6,7,8];
	exp.redMultiplier  = 2.5;
	exp.redPowers      =  round(exp.redMultiplier*64 - exp.redMultiplier.*exp.laserPowers);
	exp.opposingBlue   =  zeros(1,8);
	exp.opposingRed    =  round(exp.redMultiplier*64.*ones(1,8));
	exp.laserFilter    = 1;
	exp.nReps          = 8;
