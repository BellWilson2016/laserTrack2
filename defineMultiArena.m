function defineMultiArena(varargin)
	...
	trackingParams.laneOriginX = (rightBound(1) - leftBound(1))/2 + 1;
	trackingParams.laneOriginY = laneHalfWidthPx + 1;

	if showHalfAndCenter
		for rN = 1:8
			trackingParams.calMarks(end+1) = line( [reg(rN,1) + trackingParams.laneOriginX - 1,...
														reg(rN,1) + trackingParams.laneOriginX - 1],...
				  								   [reg(rN,3) reg(rN,4)],...
												 	'Color',.5*[1 1 1]);
			trackingParams.calMarks(end+1) = line( [reg(rN,1) reg(rN,2)],...
				 								   [reg(rN,3) + trackingParams.laneOriginY - 1, ...
 														reg(rN,3) + trackingParams.laneOriginY - 1],...
													'Color',.5*[1 1 1]);
		end
	end
